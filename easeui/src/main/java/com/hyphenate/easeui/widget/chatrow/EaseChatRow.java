    ...
    private void setUpBaseView() {
        ...
        * */
        if(userAvatarView != null) {
            ...
            if (message.direct() == Direct.SEND) {//发送方[自己]
                //将昵称和头像信息保存到本地中
                userAvatarView.setImageResource(R.drawable.p1);
                System.out.println("进来啦:"+userAvatarView);
                EaseUserUtils.setUserAvatar(context, EMClient.getInstance().getCurrentUser(), userAvatarView);
            } else {//接收方[对方]  在消息扩展中拿到头像和昵称
                userAvatarView.setImageResource(R.drawable.p2);
